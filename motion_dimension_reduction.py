...
class MotionDimensionReduction(object):
    ...
    def __init__(self, skeleton_bvh, params):
        ...
        * motion_data: dictionary
        ...
        self.repo_dir = r'D:\workspace\repo'
        ...
        animated_joints = [joint for joint in skeleton_bvh.get_animated_joints()]
        self.fdata['n_dim_spatial'] = len(animated_joints) * LEN_QUATERNION + LEN_ROOT_POSITION
        self.fdata['n_basis'] = self.params.n_basis_functions_spatial
        self.fdata['animated_joints'] = animated_joints
    ...
    def use_fpca_on_temporal_data(self, use_R=False):
        ...
        if use_R:
            temporal_fdata = self.fpca_temporal.fpca_on_temporal_data_using_R()
        else:
            temporal_fdata = self.fpca_temporal.fpca_on_temporal_data()
        return temporal_fdata

    def use_fpca_on_spatial_data(self):
        '''
        apply functional pca on spatial motion data
        :return:
        '''
    ...
    def use_sfpca_on_spatial_data(self):
        '''
        apply scaled functional pca on spatial motion data
        :return:
        '''
        if not self.spatial_data:
            raise ValueError('spatial data is not loaded!')
        ...
        smoothed_quat_frames = self.smooth_quat_frames(self.rescaled_quat_frames)
        print("smoothing is done!")
        self.sfpca_spatial = FPCASpatialData(
            n_basis=self.params.n_basis_functions_spatial,
            fraction=self.params.fraction)
        self.fpca_spatial.fit_motion_dictionary(smoothed_quat_frames)

    def get_scaled_root_fd_data(self):
        if not self.spatial_data:
            raise ValueError('spatial data is not loaded!')
        self.convert_euler_to_quat()
        self.scale_rootchannels()
        smoothed_quat_frames = self.smooth_quat_frames(self.rescaled_quat_frames)
        fd = FunctionalData()
        fd_data, original_shape, knots = fd.convert_motions_to_functional_data(smoothed_quat_frames.values(),
                                                                               self.params.n_basis_functions_spatial)
        return fd_data, original_shape, knots
    ...
    def get_backprojection_from_fpca(self):
        '''
        backporject low dimensional vector generated from fpca to original motion space
        :return:
        '''
        assert self.fpca_spatial is not None
        reconstructed_data = self.fpca_spatial.get_backprojection(self.fdata['n_frames'])
        ## rescale reconstructed data
        reconstructed_data[:, :, 0] *= self.fdata['scale_vector'][0]
        reconstructed_data[:, :, 1] *= self.fdata['scale_vector'][1]
        reconstructed_data[:, :, 2] *= self.fdata['scale_vector'][2]
        return reconstructed_data

    def use_pca_on_spatial_data(self):
        '''
        apply pca on spatial motion data
        :return:
        '''
        if not self.spatial_data:
            raise ValueError('spatial data is not loaded!')
        ...
        self.scale_rootchannels()
        smoothed_quat_frames = self.smooth_quat_frames(self.rescaled_quat_frames)
        self.pca_spatial = PCASpatialData(fraction=self.params.fraction)
        self.pca_spatial.fit_motion_dictionary(smoothed_quat_frames)

    def get_backprojection_from_pca(self):
        '''
        backporject low dimensional vector generated from pca to original motion space
        :return:
        '''
        assert self.pca_spatial is not None
        reconstructed_data = self.pca_spatial.get_backprojection()
        ## rescale reconstructed data
        reconstructed_data[:, :, 0] *= self.fdata['scale_vector'][0]
        reconstructed_data[:, :, 1] *= self.fdata['scale_vector'][1]
        reconstructed_data[:, :, 2] *= self.fdata['scale_vector'][2]
    ...
    def convert_euler_to_quat(self):
        for filename, frames in self.spatial_data.items():
            self.quat_frames[filename] = convert_euler_frames_to_quaternion_frames(self.skeleton_bvh, frames,
                                                                                   animated_joints=self.skeleton_bvh.animated_joints)
    ...
    def gen_data_for_modeling(self, use_R=False):
        print("fpca on temporal parameters")
        temporal_fdata = self.use_fpca_on_temporal_data(use_R)
        print('fpca on spatial parameters')
        self.use_fpca_on_spatial_data()
        ...
        self.fdata['n_basis_temporal'] = temporal_fdata['n_basis_temporal']
        self.fdata['temporal_eigenvectors'] = temporal_fdata['temporal_eigenvectors']
        self.fdata['temporal_parameters'] = temporal_fdata['temporal_parameters']
        self.fdata['temporal_mean'] = temporal_fdata['temporal_mean']
        self.fdata['n_dim_temporal'] = temporal_fdata['n_dim_temporal']
    ...
    @staticmethod
    def expmap_quat_motion(quat_frames):
        res = []
    ...
    def save_data(self, save_filename):
        ...
        output_data = {'n_basis_spatial': self.fdata['n_dim_spatial'],
                       ...
                       'mean_vector_spatial': self.fpca_spatial.fpcaobj.centerobj.mean.tolist(),
