...
class SFPCASpatialData(object):

    def __init__(self, n_basis, feature_weights, n_components=None, fraction=0.95):
        """
        * motion_data: dictionary
        \tContains quaternion frames of each file
        """
        assert len(feature_weights) == n_joints, ("only 19 jonits are supported!")
    ...
    def fit(self, motion_data):
        ...
        ## rescale smoothed quaternion frame with feature weights
        extended_weights = np.zeros(LEN_CARTESIAN + n_joints * LEN_QUAT)
        extended_weights[:3] = self.feature_weights[:3]
